...
function drawDrawMenu()
    ...
    sspr(sx,     sy,   8, 8, x, y_top, size, size)
    sspr(sx, sy + 8,   8, 8, x, y_bot, size, size)
    ...
    sspr(cycle({0, 8, 16, 24}, 30), 8, 8, 8, px - halfPixel, py - halfPixel, pixelSize + 1, pixelSize, flip, flip)
    pal(0)
    for i = 4, 15 do
        py = y_top + i * pixelSize + halfPixel
        rectfill(px - halfPixel, py - halfPixel, px + halfPixel, py + halfPixel, i)
        if weaponColors[i] < 0 then
            ...
            print(text, px - len/2, py - halfPixel + 2, 1)
        end
    end 
    ...
    if relx >= -pixelSize and relx < size and rely >= 0 and rely < size * 2 then
        ...
        --circfill(px, py, 2, 1)
        rect(px - halfPixel, py - halfPixel, px + halfPixel, py + halfPixel, drawColor)
    end
end
